...
def gaussian_fit(images):
	...
	for i in range(len(variance)):
		if np.all(variance[i]==0):
			zero_rows.append(i)
        new_mean = np.delete(mean, zero_rows)
        new_variance = np.delete(variance, zero_rows)
        ind_mean_dict = {}
...
def multinormalpdf((mu, Sigma, ind_mean_dict), x):
		...
		for ind in ind_mean_dict.keys():
				if np.all((x[ind] - ind_mean_dict[ind]) == 0):
						...
						new_x = np.delete(x, ind_mean_dict.keys())
						denom = np.sqrt(2*np.pi)**img_size/np.sqrt(np.linalg.det(Sigma))
						return np.exp(-0.5*(new_x-mu).T.dot(np.linalg.inv(Sigma)).dot(new_x-mu))/denom
